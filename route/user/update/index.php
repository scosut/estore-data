<?php
	...
	if (Validate::isNotEmpty($data->email, "Please enter email.")) {
		if (Validate::isValidEmail($data->email, "Please enter valid email.")) {
			Validate::toggleError($data->email, $obj->searchOtherUsersEmails($data), "Email is already taken.");
		}
	}														 
				
	if (strlen($data->password->value) > 0) {
	...
	if (Validate::isValid($data)) {
		if ($obj->updateUser($data)) {
			$user = $obj->getUserById($data->id->value);
